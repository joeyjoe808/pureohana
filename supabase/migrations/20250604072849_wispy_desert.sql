/*
  # Hero Slideshow Tables and Policies
  
  1. New Tables
    - `hero_slides` - For storing hero slideshow slides content
    - `hero_settings` - For managing hero slideshow settings
  
  2. Security
    - Enable RLS on tables
    - Add conditional policy creation to avoid errors
    - Insert default values
*/

-- Hero Slides Table (check if it exists first)
...
-- Hero Settings Table (check if it exists first)
...
-- Create RLS Policies for hero_slides conditionally
DO $$
BEGIN
...
-- Create RLS Policies for hero_settings conditionally
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT FROM pg_policies 
