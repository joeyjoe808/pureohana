...
import { supabase, checkSupabaseConnection, logSecurityEvent } from '../../lib/supabase';
import { loginRateLimiter } from '../../lib/securityConfig';
...
const AdminLogin = () => {
  ...
  const [remainingAttempts, setRemainingAttempts] = useState(5); // Default max attempts
  const [blockUntil, setBlockUntil] = useState<Date | null>(null);
  const navigate = useNavigate();
  
  // Client IP address for rate limiting (placeholder - will be replaced by server)
  const clientIdentifier = 'admin-login';
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (data.session) {
        ...
        logSecurityEvent('auth_session_verified', {
        ...
        const remaining = loginRateLimiter.getRemainingAttempts(clientIdentifier);
  ...
  const handleSubmit = async (e) => {
    ...
    // Check rate limiting
    if (!loginRateLimiter.checkLimit(clientIdentifier)) {
    ...
    try {
      // Log attempt (but mask most of the email for privacy)
      logSecurityEvent('auth_sign_in_attempt', { 
        email_partial: email.slice(0, 3) + '***',
        source: 'admin_login_page' 
      });
      ...
      if (error) {
        ...
        if (error.message.includes('Invalid login credentials')) {
          throw new Error('Invalid email or password. Please verify your credentials or run the admin user creation script.');
        } else if (error.status === 403 || error.code === 'PGRST301') {
          throw new Error('Access forbidden. Your IP may be blocked or you may need to configure CORS settings in Supabase.');
        } else {
          throw error;
        }
      }
      ...
      if (data && data.session) {
        ...
        logSecurityEvent('auth_sign_in_success', { 
          user_id: data.session.user.id,
          source: 'admin_login_page' 
        });
        
        // Reset rate limiting on successful login
        loginRateLimiter.clearLimit(clientIdentifier);
        
        // Force session refresh to ensure we have the latest session
        await supabase.auth.refreshSession();
        
        // Navigate after successful login with a slight delay to allow session to be set
        setTimeout(() => {
      ...
      logSecurityEvent('auth_sign_in_failed', { 
        email_partial: email.slice(0, 3) + '***',
        error_type: errorMessage.includes('Invalid') ? 'invalid_credentials' : 'other',
        source: 'admin_login_page' 
      });
      ...
      if (remaining <= 0) {
        // Calculate when the user can try again (1 hour from now)
          ...
          <div className="mb-6 bg-blue-500/10 border border-blue-500/30 rounded-md p-4 flex items-start">
            ...
            <div>
              ...
              <ol className="text-xs text-blue-300 mt-2 list-decimal pl-4 space-y-1">
                <li>Go to your <a href="https://app.supabase.com" target="_blank" rel="noreferrer" className="underline hover:text-blue-200 flex items-center inline-flex">Supabase Dashboard<ExternalLink size={10} className="ml-1" /></a></li>
          ...
          {error && (
            <div className="mb-6 bg-red-500/10 border border-red-500/30 rounded-md p-4 flex items-start">
              ...
              <div>
                ...
                {blockUntil && (
                  <p className="text-xs text-red-300 mt-2">
                    Too many failed attempts. Please try again after {blockUntil.toLocaleTimeString()}.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <button
                  type="submit"
                  disabled={loading || remainingAttempts <= 0}
                  className="w-full bg-yellow-400 text-slate-900 py-3 px-4 rounded hover:bg-yellow-500 transition-colors font-medium flex justify-center items-center disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? (
